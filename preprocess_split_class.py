...
image_path = glob.glob('saving/trash_split_class/*') #->pet, can
train_raw_path = 'saving/raw/cut_100/train/'
test_raw_path = 'saving/raw/cut_100/test/'

train_npy_path = 'saving/npy/cut_100/x_train{}.npy'
train_label_npy_path = 'saving/npy/cut_100/x_train_label{}.npy'

test_npy_path = 'saving/npy/cut_100/x_test{}.npy'
test_label_npy_path = 'saving/npy/cut_100/x_test_label{}.npy'
...
def preprocess(image_path, train_raw_path, test_raw_path,
    ...
    data_path = []
    for path in image_path: #->path: 폴더 주소
        #print(path)
        class_directory = glob.glob(path+'/*')
        for trashes_directory in class_directory:
            #print(trashes_directory)
            trashes = glob.glob(trashes_directory+'/*') #이미지 리스트
            #print(trashes)
            '''
            ...
            '''
            #print(trashes_list)
            name = os.path.basename(trashes_directory)
            #print(name)
            name_list = [name for i in range(len(trashes))]
            #print(name_list)
            data_path += list(zip(trashes, name_list))
            #print(list(zip(trashes_list, name_list)))
